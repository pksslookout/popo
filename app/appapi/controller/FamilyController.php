...
class FamilyController extends HomebaseController {
	...
	function home(){
		...
		if($familyinfo){
			...
			$this->assign("language_type",$lang);
	...
	public function add()
	{
        ...
        if($fullname==''){
            echo '{"state":"0","msg":"'.lang('请填写个人姓名').'"}';
			exit;
        }
        
        if($carded==''){
            echo '{"state":"0","msg":"'.lang('请填写身份证号').'"}';
			exit;
        }
        
        if($apply_pos==''){
            echo '{"state":"0","msg":"'.lang('请上传身份证正面照').'"}';
			exit;
        }
        
        if($apply_side==''){
            echo '{"state":"0","msg":"'.lang('请上传身份证背面照').'"}';
			exit;
        }
        
        if($apply_map==''){
            echo '{"state":"0","msg":"'.lang('请上传家族图标').'"}';
			exit;
        }
		...
		if($family){
			if($family['state']==0){
				echo '{"state":"0","token":"'.$token.'","uid":"'.$uid.'","msg":"'.lang('您提交的家族申请正在审核中...').'"}';
				exit;
			}
            
            if($family['state']==2){
				echo '{"state":"0","token":"'.$token.'","uid":"'.$uid.'","msg":"'.lang('家族已创建成功').'"}';
				exit;
			}
