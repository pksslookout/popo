...
class AuthController extends Controller {
	...
	public function authsave(){
		...
		if( !$uid || !$token || checkToken($uid,$token)==700 ){
            echo json_encode(array("ret"=>0,'data'=>array(),'msg'=>lang('您的登陆状态失效，请重新登陆！')));
			exit;
		} 
        ...
        if($real_name==''){
            echo json_encode(array("ret"=>0,'data'=>array(),'msg'=>lang('请填写您的真实姓名')));
			exit;
        }
        
        if($mobile==''){
            echo json_encode(array("ret"=>0,'data'=>array(),'msg'=>lang('请填写您的手机号')));
			exit;
        }
        
        if($cer_no==''){
            echo json_encode(array("ret"=>0,'data'=>array(),'msg'=>lang('请填写您的身份证号')));
			exit;
        }
        
        if($front_view==''){
            echo json_encode(array("ret"=>0,'data'=>array(),'msg'=>lang('请上传证件相关照片')));
			exit;
        }
		...
		if($result!==false){
			...
			echo json_encode(array("ret"=>0,'data'=>array(),'msg'=>lang('提交失败，请重新提交')));
		}
        exit;
	}	
	/* 成功 */
