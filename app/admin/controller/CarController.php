...
class CarController extends AdminbaseController {
	...
	function del(){
        ...
        delete_language_translate('car', 'name', $id);
        $action="删除坐骑：{$id}";
        setAdminLog($action);
	...
	function addPost(){
		if ($this->request->isPost()) {
            ...
            insert_language_translate('car', 'name', $id, $language);
            $action="添加坐骑：{$id}";
            setAdminLog($action);
	...
	function edit(){
        ...
        $this->assign('list_language_translate', get_language_translate('car', 'name', $id));
	...
	function editPost(){
		if ($this->request->isPost()) {
            ...
            update_language_translate('car', 'name', $data['id'], $language);
            $action="修改坐骑：{$data['id']}";
            setAdminLog($action);
