...
class GuardController extends AdminbaseController {
	...
	function del(){
        ...
        delete_language_translate('guard', 'name', $id);
        $action="删除守护：{$id}";
        setAdminLog($action);
	...
	function addPost(){
		if ($this->request->isPost()) {
            ...
            insert_language_translate('guard', 'name', $id, $language);
    ...
    function edit(){
        ...
        $this->assign('type_a', $this->getTypes());
        $this->assign('length_type_a', $this->getLengthtype());
        $this->assign('list_language_translate', get_language_translate('guard', 'name', $id));
        return $this->fetch();
    }
