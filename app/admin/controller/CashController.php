...
class CashController extends AdminbaseController {
    ...
    protected function getCashType($k=''){
        $cashType=array(
            '0'=>'普通提现',
            '1'=>'视频红包提现',
        );
        ...
        return isset($cashType[$k]) ? $cashType[$k]: '';
    }
...
//            $map[]=['uid|orderno|trade_no','like',"%".$keyword."%"];
        ...
        if($status=='')
        {
            ...
            $fail=$map;
            $fail[]=['status','=',2];
            $cashrecord_wait = DB::name("cash_record")->where($wait)->sum("money");
    	...
    	$this->assign('cashType', $this->getCashType());
