...
class VipchargeController extends AdminbaseController {
    ...
    function index(){
        ...
        $moneysum = Db::name("vip_charge_user")
            ->where($map)
			->sum('money');
        if(!$moneysum){
            $moneysum=0;
    ...
    function setPay(){
        ...
        if($id){
            $result=Db::name("vip_charge_user")->where(["id"=>$id,"status"=>0])->find();
            if($result){
                ...
                /* 更新 订单状态 */
                Db::name("vip_charge_user")->where("id='{$result['id']}'")->update(array("status"=>1));
    ...
    function del(){
        ...
        $rs = DB::name('vip_charge_user')->where("id={$id}")->delete();
        if(!$rs){
            $this->error("删除失败！");
        }
    ...
    function export()
    {
        ...
        $xlsData=Db::name("vip_charge_user")
        ...
        $action="导出充值记录：".Db::name("vip_charge_user")->getLastSql();
        setAdminLog($action);
