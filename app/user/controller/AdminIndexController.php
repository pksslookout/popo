...
class AdminIndexController extends AdminBaseController{
    ...
    public function index(){
        ...
        $isanchor=isset($data['isanchor']) ? $data['isanchor']: '';
        if($isanchor!=''){
...
//        delIMUser($id);
    ...
    function setanchor(){
        ...
        $id = $this->request->param('id', 0, 'intval');
        $isanchor = $this->request->param('isanchor', 0, 'intval');

        $rs = DB::name('user')->where("id={$id}")->setField('isanchor',$isanchor);
        if(!$rs){
            $this->error("操作失败！");
        }
    ...
    function setzombieall(){
        ...
        $rs = DB::name('user')->where('user_type=2 and iszombie='.$up_iszombie)->limit(1000)->setField('iszombie',$iszombie);
        if(!$rs){
            $this->error("操作失败！");
        }
	...
	function editvip(){
        ...
        $data['userinfo']=getUserInfo($data['id']);

        $data_vip=Db::name('vip_user')
            ->where("uid={$id}")
            ->find();
        if(!$data_vip){
            $data_vip['endtime'] = '';
        }else{
            $data_vip['endtime'] = date('Y-m-d', $data_vip['endtime']);
        }
        $this->assign('endtime', $data_vip['endtime']);
        $this->assign('data', $data);
        return $this->fetch();
	}
	
	function editvipPost(){
        if ($this->request->isPost()) {
            ...
            if($isexist2){
                $datavip=[];
                $datavip['endtime']=strtotime($data['endtime']);
                $rs = DB::name('vip_user')->where("uid={$uid}")->update($datavip);
                if(!$rs){
                    $this->error("修改失败！");
                }
                $action="修改用户VIP：{$uid}";
