...
d.on("error", function(err) {
 ...
 // var options = {
 //     key: fs.readFileSync('/usr/local/nginx/conf/ssl/test.popolive.net.key'),
 //     cert: fs.readFileSync('/usr/local/nginx/conf/ssl/test.popolive.net.crt')
   // }; 
...
io.on('connection', function(socket) {
	...
	socket.on('conn', function(data) {
        ...
        // const existingSocket = io.sockets.connected[clientIp];
        // if (existingSocket) {
        //     existingSocket.disconnect(); // 断开现有的连接
        // }

        if (io.sockets.adapter.rooms[`/${userId}`]) {
            socket.disconnect(); // 如果已经有这个用户的连接，断开新的连接
        } else {
            socket.join(userId); // 否则，加入房间以跟踪连接
            // 其他逻辑
        }
