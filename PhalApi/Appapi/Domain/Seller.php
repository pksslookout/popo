...
class Domain_Seller {
	...
	public function getExpressList(){
        ...
        if(!in_array($lang,['zh_cn','en'])) {
            $translate = get_language_translate('shop_express', 'express_name', $lang);
        }
		foreach ($express_list as $k => $v) {
			$express_list[$k]['express_thumb']=get_upload_path($v['express_thumb']);
            if($lang=='en'){
                $express_list[$k]['express_name']=$v['express_name_'.$lang];
            }else{
                if($lang!='zh_cn'){
                    if(isset($translate[$v['id']])){
                        $express_list[$k]['express_name']=$translate[$v['id']];
                    }
                }
            }
		}
