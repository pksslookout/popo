...
class Api_Paidprogram extends PhalApi_Api {
	...
	public function getApplyStatus(){
        ...
        $rs['info'][0]['isauth']='1';
        $rs['info'][0]['auth_msg']=T('申请开通付费内容权限需要先进行实名认证');
		...
		switch ($apply_status) {
			...
			case '-2': //没申请
				...
				$payment_title=T('《平台付费内容管理规范协议说明》');
				break;

			case '-1':
				$title=T('申请未通过');
				$desc=T('您的申请被拒,{payment_time}日后可再次申请,如有疑问可咨询平台客服',['payment_time'=>$configpub['payment_time']]);
				break;

			case '0':
				$title=T('申请已提交');
				$desc=T('审核通过后即可上传付费内容');
				break;
	...
	public function apply() {
		$rs = array('code' => 0, 'msg' => T('申请成功'), 'info' => array());
		...
		if($res==1001){
			$rs['code']=1001;
			$rs['msg']=T('申请已通过,不可重复申请');
			return $rs;
		}else if($res==1002){
			$rs['code']=1002;
			$rs['msg']=T('正在审核中,请耐心等待');
			return $rs;
		}else if($res==1003){
			$rs['code']=1003;
			$rs['msg']=T('申请太频繁,请过段时间再试');
			return $rs;
		}else if($res==1004){
			$rs['code']=1004;
			$rs['msg']=T('申请失败,请重试');
	...
	public function addPaidProgram(){
		$rs = array('code' => 0, 'msg' => T('付费内容发布成功'), 'info' => array());
		...
		if($type==0&&$count>1){
			$rs['code']=1001;
			$rs['msg']=T('单视频只允许上传一个视频');
			return $rs;
		}

		if($type==1&&$count==1){
			$rs['code']=1001;
			$rs['msg']=T('多视频不允许上传一个视频');
			return $rs;
		}
    ...
    public function getAliOrder(){
    	$rs = array('code' => 0, 'msg' => T('订单创建成功'), 'info' => array());
    ...
    public function getWxOrder(){
		...
		$paramarr = array(
			...
			"body"        =>    T("花费").$paidprogram_info['money'].T("购买").$paidprogram_info['title'],
	...
	*/
	...
	public function setComment(){
		$rs = array('code' => 0, 'msg' => T('评价成功'), 'info' => array());
        ...
        if($res==1004){
        	$rs['code']=1004;
            $rs['msg']=T('不能对自己的付费项目发表评价');
            return $rs;
        }

        if($res==1005){
        	$rs['code']=1005;
            $rs['msg']=T('不能对未审核通过的付费项目发表评价');
            return $rs;
        }
