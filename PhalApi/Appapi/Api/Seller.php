...
class Api_Seller extends PhalApi_Api {
	...
	public function setGoods(){
		...
		$rs = array('code' => 0, 'msg' => T('商品发布成功'), 'info' => array());
		...
		if($specname_len_false){
			$rs['code']=1001;
			$rs['msg']=T('商品规格名称应在15个字符以内');
			return $rs;
		}

		if($specnum_false){
			$rs['code']=1001;
			$rs['msg']=T('商品规格库存在1-9999999之间');
			return $rs;
		}

		if($specprice_false){
			$rs['code']=1001;
			$rs['msg']=T('商品规格单价在1-99999999之间');
			return $rs;
		}
		...
		if($postage<0||$postage>99999){
			$rs['code']=1001;
			$rs['msg']=T('商品运费在0-99999之间');
			return $rs;
		}

        if($share_income<0||$share_income>99999){
            $rs['code']=1001;
            $rs['msg']=T('分享佣金在{num1}-{num2}之间',['num1'=>0,'num2'=>99999]);
            return $rs;
        }
	...
	public function upReceiverAddress(){
		$rs = array('code' => 0, 'msg' => T('退货地址修改成功'), 'info' => array());
	...
	public function upGoodsSpecs(){
		$rs = array('code' => 0, 'msg' => T('商品规格更新成功'), 'info' => array());
...
//		}
		...
		if($res==1003){
			$rs['code']=1003;
			$rs['msg']=T('商品审核中,无法修改规格');
			return $rs;
		}

		if($res==1004){
			$rs['code']=1004;
			$rs['msg']=T('商品下架,无法修改规格');
			return $rs;
		}
	...
	public function upGoods(){
		$rs = array('code' => 0, 'msg' => T('商品修改成功'), 'info' => array());
		...
		if($postage<0||$postage>999){
			$rs['code']=1001;
			$rs['msg']=T('商品运费在0-999之间');
			return $rs;
		}

        if($share_income<0||$share_income>999){
            $rs['code']=1001;
            $rs['msg']=T('分享佣金在{num1}-{num2}之间',['num1'=>0,'num2'=>999]);
            return $rs;
        }
	...
	public function delGoods() {
		$rs = array('code' => 0, 'msg' => T('商品删除成功'), 'info' => array());
	...
	public function setExpressInfo(){
		$rs=array('code'=>0,'msg'=>T('订单发货成功'),'info'=>array());
		$uid=checkNull($this->uid);
		$token=checkNull($this->token);
    ...
    public function delGoodsOrder(){
        ...
        $rs=array('code'=>0,'msg'=>T('订单删除成功'),'info'=>array());
    ...
    public function getGoodsOrderInfo(){
        ...
        if($order_info['status']==1){ //订单待发货
        	$order_info['status_name']=T('待发货');
        	$order_info['status_desc']=T('请尽快发货,逾期将会自动关闭订单,钱款会退还给买家');
        }

        if($order_info['status']==2){ //订单已发货
        	$order_info['status_name']=T('已发货');
        }

        if($order_info['status']==3&&$order_info['settlement_time']==''){ //已签收 未结算
        	$order_info['status_name']=T('已签收');
        	$order_info['status_desc']=T('商品钱款会尽快核算发放');
        }

        if($order_info['status']==3&&$order_info['settlement_time']!=''){ //已签收 已结算
        	$order_info['status_name']=T('交易成功');
        	$order_info['status_desc']=T('商品钱款已发放到您的账户中');
        }
    ...
    public function getGoodsOrderRefundInfo(){
        ...
        if($refund_info['status']==-1){ //买家取消
        	...
        	if($is_platform_interpose==0){ //平台未介入
        		...
        		if($refund_shop_result==0){
	                ...
	                $status_name=T('买家申请退款');
	                $end=$refund_info['addtime']+$effective_time['shop_refund_finish_time']*24*60*60;
	                $cha=$end-$now;
	                $status_time=T('还剩').getSeconds($cha);
	                $can_handle='1';

	                if($shop_process_num>0){
	                	$status_name=T('买家重新申请退款');
	                }

	            }elseif($refund_shop_result==-1){ //卖家拒绝

	                $status_name=T('您已经拒绝了买家退款申请{shop_process_num}次',['shop_process_num'=>$shop_process_num]);
	                if($shop_process_num<3){ 
	                	$status_desc=T('买家可重新提交申请或申请平台介入');
	                	$can_handle='0';

	                }else{
	                	$status_desc=T('买家可申请平台介入');
	                }
            ...
            if($is_platform_interpose){ //平台介入
                ...
                if($refund_shop_result==1){ //卖家同意
                   ...
                   $status_desc=T('您已同意退款,退款金额将退回到买家账户余额');
                }elseif($refund_shop_result==-1){ //卖家拒绝
                    $status_name=T('您已经拒绝了买家退款申请');
                    $status_time=date("Y-m-d H:i:s",$refund_info['system_process_time']); //系统自动处理时间
                    $status_desc=T('您已拒绝退款,拒绝原因:');
                }
        ...
        $buyer_info=getUserInfo($refund_info['uid'],1);
    ...
    public function setGoodsOrderRefund(){
    	$rs=array('code'=>0,'msg'=>T('退款处理完成'),'info'=>array());
        ...
        if($refund_status==1){ //已经完成
        	...
        	if($shop_result==-1){
        		$rs['code']=1001;
	            $rs['msg']=T('您已经拒绝,不能再操作');
	            return $rs;
        	}elseif($shop_result==1){
        		$rs['code']=1001;
	            $rs['msg']=T('您已经同意,不能再操作');
	            return $rs;
        	}

        	if($shop_process_num>=3){
        		$rs['code']=1001;
	            $rs['msg']=T('您已经拒绝{shop_process_num}次,不能再操作',['shop_process_num'=>$shop_process_num]);
	            return $rs;
        	}
...
//            $title=T("你的商品“{goods_name}”卖家已经同意退款",['goods_name'=>$order_info['goods_name']]);
...
//        	$title=T("你的商品“{goods_name}”卖家拒绝退款",['goods_name'=>$order_info['goods_name']]);
