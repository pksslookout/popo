...
class Api_Buyer extends PhalApi_Api {
     ...
     * @return int info[0]['wait_evaluate'] 待评价订单数
	...
	public function addAddress() {
		$rs = array('code' => 0, 'msg' => T('地址添加成功'), 'info' => array());
    ...
    public function addGoodsVisitRecord(){
        ...
        $rs = array('code' => 0, 'msg' => T('商品浏览记录添加成功'), 'info' => array());
    ...
    public function delGoodsVisitRecord(){
        ...
        $rs = array('code' => 0, 'msg' => T('商品浏览记录删除成功'), 'info' => array());
    ...
    public function createGoodsOrder(){
        ...
        try {
            ...
            //减少商品规格库存量
            $isok=changeShopGoodsSpecNumV2($order_data['goodsid'], $specs_data, $order_data['spec_id'], $order_data['nums'], 0);
            if(!$isok){
                DI()->notorm->rollback('db_appapi');
                $rs['code'] = 1001;
                $rs['msg'] = T('没有对应的商品规格');
...
//        }
		...
		switch($type){
			...
			case '2': //微信
				...
				$paramarr = array(
					...
					"body"        =>    T("支付").$order_info['total'].T("购买").filterEmoji($order_info['goods_name']),
            ...
            case '4'://微信小程序
                ...
                $paramarr = array(
                    ...
                    "body"        =>     T("支付").$order_info['total']. T("购买").$order_info['goods_name'],
    ...
    protected function createGoodsOrderTest($data){
        ...
        $domain_shop=new Domain_Shop();
        $goods_info=$domain_shop->getGoodsIdLock($goodsid);
        if(!$goods_info){
        	$rs['code'] = 1003;
            $rs['msg'] =  T('商品不存在');
            return $rs;
	...
	*/
    ...
    public function receiveGoodsOrder(){
        $rs=array('code'=>0,'msg'=> T('确认收货成功'),'info'=>array());
    ...
    public function cancelRefundGoodsOrder(){
        ...
        $rs=array('code'=>0,'msg'=>T('订单退款取消成功'),'info'=>array());
    ...
    public function reapplyRefundGoodsOrder(){
        $rs=array('code'=>0,'msg'=>T('重新申请成功'),'info'=>array());
     ...
     * @return array info[0]['list'][]['balance'] 金额
     * @return array info[0]['list'][]['addtime'] 添加时间
     */
    public function getShopCashList(){
        $rs=array('code'=>0,'msg'=>'','info'=>array());
        ...
        $domain=new Domain_Buyer();
        $res=$domain->getShopCashList($uid,$p);
