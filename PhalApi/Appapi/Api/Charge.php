...
class Api_Charge extends PhalApi_Api {
	...
	public function getVipWxOrder() {
		...
		$domain = new Domain_Charge();
		$info = $domain->getVipOrderId($changeid,$orderinfo);
		if($info==1003){
			$rs['code']=1003;
			$rs['msg']= T('订单信息有误，请重新提交');
            return $rs;
		...
		$paramarr = array(
			"appid"       =>   $configpri['wx_appid'],
			"body"        =>     T("购买{name}会员",array("name"=>$info['name'])),
			"mch_id"      =>    $configpri['wx_mchid'],
			"nonce_str"   =>    $noceStr,
			"notify_url"  =>    $configpub['site'].'/Appapi/pay/notify_wx',
			"out_trade_no"=>    $orderid,
	...
	*/
	 ...
	 * @return string msg 提示信息
	 */
	public function getVipAliOrder() {
		$rs = array('code' => 0, 'msg' => '', 'info' => array());
	 ...
	 * @return string msg 提示信息
	 */
	public function getVipIosOrder() {
		$rs = array('code' => 0, 'msg' => '', 'info' => array());
