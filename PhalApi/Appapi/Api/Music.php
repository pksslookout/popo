...
class Api_Music extends PhalApi_Api {
	...
	public function getRules() {
        return array(
            ...
            'getMusicInfo' => array(
                'music_id' => array('name' => 'music_id', 'type' => 'string','require' => true,'desc' => '音乐ID'),
                'uid' => array('name' => 'uid', 'type' => 'int','desc' => '用户id'),
            ),
            'collectMusic'=>array(
     ...
     * @return string msg 提示信息
     * @return array info 返回信息
     * @return string info[.title 音乐名称
     * @return string info.author 演唱者
     * @return string info.img_url 封面
     * @return string info.length 音乐长度
     * @return string info.file_url 音乐文件地址
     * @return int info.use_nums 音乐被使用次数
     * @return int info.iscollect 音乐是否被该用户收藏
     */
    public function getMusicInfo(){
        ...
        $domain=new Domain_Music();
        $res=$domain->getMusicInfo($music_id,$uid);
