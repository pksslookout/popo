...
class Model_Seller extends PhalApi_Model_NotORM {
	...
	public function getGoodsClass($uid) {
        ...
        if(!$class_list){
        	...
        	//获取一级分类
        	$class_list=DI()->notorm->shop_goods_class->select("gc_id,gc_name,gc_name_en")->where("gc_parentid=0 and gc_isshow=1")->order("gc_sort")->fetchAll();

        	foreach ($class_list as $k => $v) {
	            //获取二级分类

	            $two_list=DI()->notorm->shop_goods_class->select("gc_id,gc_name,gc_name_en")->where("gc_parentid=? and gc_isshow=1",$v['gc_id'])->order("gc_sort")->fetchAll();

	            if($two_list){
	                foreach ($two_list as $k1 => $v1) {
	                    //查询三级分类
	                    $three_list=DI()->notorm->shop_goods_class->select("gc_id,gc_name,gc_name_en")->where("gc_parentid=? and gc_isshow=1",$v1['gc_id'])->order("gc_sort")->fetchAll();
        ...
        foreach ($seller_class_list as $k => $v) {
            foreach ($v['two_list'] as $k1 => $v1) {
                if($lang=="en"){
                    ...
                    if($lang!='zh_cn'){
                        if(isset($translate[$v1['gc_id']])){
                            $v1['gc_name']=$translate[$v1['gc_id']];
                        }
                    }
                }
                foreach ($v1['three_list'] as $k2 => $v2) {
                    if($lang=='en'){
                        ...
                        if($lang!='zh_cn'){
                            if(isset($translate[$v2['gc_id']])){
                                $v2['gc_name']=$translate[$v2['gc_id']];
                            }
                        }
                    }
                    $v1['three_list'][$k2]=$v2;
        ...
        foreach ($list as $k => $v) {
            ...
            switch ($v['status']) {
                ...
                case '1':
                    ...
                    $list[$k]['status_name']=T('待发货');
                    break;

                case '2':
                    
                    $list[$k]['status_name']=T('已发货');
                    break;

                case '3':
                    
                    $list[$k]['status_name']=T('已签收');
                    break;
                ...
                case '5':
                    ...
                    if($v['refund_status']==0){
                        $list[$k]['status_name']=T('退款中');

                        if($v['refund_shop_result']==-1){
                            $list[$k]['status_name']=T('已拒绝');
                        }
