...
class Model_Dynamic extends PhalApi_Model_NotORM {
	...
	public function getDynamicLabels($p) {
		...
		foreach($reportlist as $k=>$v){
			...
			$v['name']='#'.$v['name'];
			$v['use_nums_msg']=$v['use_nums'].T('人参与了该话题');
			$reportlist[$k]=$v;
		}
		return $reportlist;
	...
	/* 获取话题标签详情 */
	public function getLabelDynamicInfo($uid,$dynamic_label_id){

		$where="id={$dynamic_label_id}";
		$dynamicInfo=DI()->notorm->dynamic_label
				->select("*")
				->where($where)
				->fetchOne();
        ...
        if($lang=='en'){
            $dynamicInfo['name']=$dynamicInfo['name_'.$lang];
        }else{
            if($lang!='zh_cn'){
                if(isset($translate[$v['id']])){
                    $dynamicInfo['name']=$translate[$dynamic_label_id];
                }
            }
        }

        $dynamicInfo['thumb']=get_upload_path($dynamicInfo['thumb']);
        $where="dynamic_label_id={$dynamic_label_id} and isdel=0 and status=1";
        $dynamicInfo['video_count']=DI()->notorm->video
            ->select("id")
            ->where($where)
            ->count();
