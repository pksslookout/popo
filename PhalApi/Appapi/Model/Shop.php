...
class Model_Shop extends PhalApi_Model_NotORM {
    ...
    public function getOneGoodsClass(){
        ...
        $list=DI()->notorm->shop_goods_class->select("gc_id,gc_name,gc_name_en,gc_isshow")->where("gc_id",$ids)->where(" gc_isshow=1")->order("gc_sort")->fetchAll();
        $lang=GL();
            ...
            if($lang=='en'){
                ...
                if($lang!='zh_cn'){
                    if(isset($translate[$v['gc_id']])){
                        $list[$k]['gc_name']=$translate[$v['gc_id']];
                    }
                }
            }
        }
	...
	public function getShop($uid,$fields='') {
        ...
        if($uid==1){ //平台自营店铺
            ...
            $shop_info['user_nicename']=T("平台自营");
            $shop_info['name']=$configpub['site_name'].T('小店');
        }else{

            //获取用户信息
            $userinfo=getUserInfo($uid,1);
            $shop_info['user_nicename']=$userinfo['user_nicename']; //用于进入私信聊天顶部显示昵称
            $shop_info['name']=$userinfo['user_nicename'].T('的小店');
        }
    ...
    /* 获取商品信息 悲观锁 */
    public function getGoodsIdLock($id){
        ...
        if($id)
        {
            ...
            $info = $this->getORM()->queryAll($sql)[0];
        }
    ...
    public function getGoodsCommentList($uid,$goodsid,$type,$p){
        ...
        foreach ($list as $k => $v) {
            ...
            if($append_comment){
                ...
                if($cha<24*60*60){
                    $append_comment['date_tips']=T('当日评论');
                }else{
                    
                    $append_comment['date_tips']=floor($cha/(24*60*60)).T('天后评论');
                }
		...
		if($info){
			...
			foreach ($classid_arr as $k => $v) {
				$class_info=DI()->notorm->shop_goods_class
					...
					->fetchOne();
				if($class_info){
                    foreach ($class_info as $k1=>$v1){
                        if($lang=='en'){
                            $class_info[$k1]['gc_name']=$v1['gc_name_'.$lang];
                        }else{
                            if($lang!='zh_cn'){
                                if(isset($translate[$v['gc_id']])){
                                    $class_info[$k1]['gc_name']=$translate[$v1['gc_id']];
                                }
                            }
                        }
                    }
